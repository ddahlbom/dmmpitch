...
if __name__=="__main__":
    ...
    if TEST:
        ...
        with torch.no_grad():
            ...
            recall = 0.0000001 if recall == 0 else recall
            precision = 0.0000001 if precision == 0 else precision
    ...
    # plt.ylim([0,1.1])
    fig = plt.figure()
    local_weight = net.fc1.weight.detach().to('cpu').numpy()
    plt.imshow(local_weight)
    fig = plt.figure()
    local_weight = net.fc2.weight.detach().to('cpu').numpy()
    plt.imshow(local_weight)
    fig = plt.figure()
    local_weight = net.fc3.weight.detach().to('cpu').numpy()
    plt.imshow(local_weight)
    fig = plt.figure()
    local_weight = net.fc4.weight.detach().to('cpu').numpy()
    plt.imshow(local_weight)
    fig = plt.figure()
    local_weight = net.fc5.weight.detach().to('cpu').numpy()
    plt.imshow(local_weight)
    plt.show()
