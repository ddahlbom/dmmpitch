...
def main(args):
    ...
    # num_test_seqs = 3
    lengths = np.zeros(num_test_seqs)
    p_vals = np.zeros(num_test_seqs)
    r_vals = np.zeros(num_test_seqs)
    f_vals = np.zeros(num_test_seqs)
    correct_vals = np.zeros(num_test_seqs)

    for seq_idx in range(num_test_seqs):
        t1 =time.strftime('%X')
        # seq_idx = np.random.randint(num_test_seqs) 
        # seq_idx = 22 
        print("Using test sequence number {}".format(seq_idx))
        seq_len = test_seq_lengths[seq_idx].item()
        lengths[seq_idx] = seq_len      #redundant, idiot!
        piano_roll_gt = test_data_sequences[seq_idx,:seq_len,:].data.numpy()
            ...
            if good_samples[f] > 0:
                ...
                if condense:
                    if np.array_equal(gt[f,:], piano_roll_estimated_c[f,:]):
                        num_chosen_c += 1
        correct_chosen_rate = num_chosen/num_available
        print("Correct select rate (normal)   : {}".format(correct_chosen_rate))
        correct_vals[seq_idx] = correct_chosen_rate
    ...
    print("Final values on set: ")
    total_steps = np.sum(lengths)
    p_final = np.dot(lengths, p_vals)/total_steps
    r_final = np.dot(lengths, r_vals)/total_steps
    f_final = np.dot(lengths, f_vals)/total_steps
    correct_final = np.dot(lengths, correct_vals)/total_steps
    print("Precision:    ", p_final)
    print("Recall:       ", r_final)
    print("F-metric:     ", f_final)
    print("Correct rate: ", correct_final)
    # plt.tight_layout()
    # plt.show()
